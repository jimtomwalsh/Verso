...
function buildBootstrap(config, registryJson, principal) {
  ...
  if (principal) {
    ...
    // The doc-of-record, inlined. Absent (rather than empty) on a fresh store, so the
    // client can tell "server has nothing yet" from "server did not answer".
    if (registryJson != null) {
      lines.push("window.__versoServerRegistryB64 = " + jsLit(Buffer.from(String(registryJson), "utf8").toString("base64")) + ";");
    }
  }
  return lines.join("\n") + "\n";
}
function sendBootstrap(res, config, store, principal) {
  var registryJson = null;
  if (principal) { try { registryJson = store.getRegistry(); } catch (e) { registryJson = null; } }
  var body = buildBootstrap(config, registryJson, principal);
  res.writeHead(200, {
...
function makeHandler(store, config, blockStore, sync, identity, review) {
  ...
  return function handler(req, res) {
    ...
    if (url === "/api/bootstrap.js") {
      ...
      return sendBootstrap(res, config, store, bp);
    }
...
module.exports = { createServer: createServer, makeHandler: makeHandler, startServer: startServer, buildBootstrap: buildBootstrap };
