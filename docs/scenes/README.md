...
| `shoot`  | `out` (`*.webp`), `clip?` | screenshot to `docs/assets/<out>`; `clip` crops the shot (see below) |
...
## Wiring a still into the guide

Reference the emitted asset from `docs/USER-GUIDE.md` with the figure directive (#25):

```
![Alt text for the figure](docs/assets/workspace-overview.webp "Caption shown under the image")
```
