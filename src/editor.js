...
(function () {
  ...
  // arch-P3b-08 (folding in 03b): the three panels for a selection that is not one block -- the
  // whole DOCUMENT, one PAGE, and a MULTI-selection -- moved to editor/inspector/scopes.js. They
  // are the last named panels that were still in this file; the dispatch table above stays,
  // because a ten-entry map of one-line forwarders is wiring, which is what this file is.
  var renderMultiInspector = VE.bind("renderMultiInspector");
  var renderPageInspector = VE.bind("renderPageInspector");
  var renderDocumentInspector = VE.bind("renderDocumentInspector");
  ...
  // ...continues in scopes.js (arch-P3b-07).
  ...
  // arch-P3b-07shell: the frame around the work -- the top bar and its pipeline buttons, the left
  // rail, the three-stage switch, the Product picker and the matrix-cell chip -- moved to
  // editor/shell.js. It came out from under a banner titled "Project auto-backup", whose actual
  // backup writer left in 07d; what was left under it was this, and it is not data-safety at all.
  var renderPipelineButtons = VE.bind("renderPipelineButtons");
  var renderToolbarPipeline = VE.bind("renderToolbarPipeline");
  var publishQueueFormat = VE.bind("publishQueueFormat");
  var openPublishFormatMenu = VE.bind("openPublishFormatMenu");
  var mountTopBar = VE.bind("mountTopBar");
  var isValidStage = VE.bind("isValidStage");
  var stageWorkspaceClass = VE.bind("stageWorkspaceClass");
  var productSelectOptions = VE.bind("productSelectOptions");
  var setStage = VE.bind("setStage");
  var activeStage = VE.bind("activeStage");
  var mountLeftRail = VE.bind("mountLeftRail");
  var currentCell = VE.bind("currentCell");
  var applyCellChange = VE.bind("applyCellChange");
  var setCellInteractive = VE.bind("setCellInteractive");
  ...
  // arch-P3b-07parts: the FIELD inspector (a text slot inside a block) and the INSTANCE inspector
  // (one card of a component grid) moved to editor/inspector/parts.js. Both are panels for a thing
  // the document model gives no id to, so both are addressed by parent-plus-index and both have to
  // re-find themselves after the parent redraws. The reselect helpers they use stayed here: three
  // other surfaces call them.
  var caretInList = VE.bind("caretInList");
  var renderFieldInspector = VE.bind("renderFieldInspector");
  var renderInstanceInspector = VE.bind("renderInstanceInspector");
  var reflectStatus = VE.bind("reflectStatus");
  ...
  window.Editor = {
    ...
    registerPipelineButton: function (label, onClick, accent, opts) {
      ...
      if (activeStage() === "source") renderSourceToolbar();
    },
  ...
  window.VersoEditor.provide({
    // @p07-provide
    loadGuide: loadGuide,
    detachPageLibraryInstance: detachPageLibraryInstance,
    TEXT_STYLE_TYPES: TEXT_STYLE_TYPES,
    collectPageOverridableTextFields: collectPageOverridableTextFields,
    reconcilePageOverrides: reconcilePageOverrides,
    moveToChapter: moveToChapter,
    applyStyleToBlock: applyStyleToBlock,
    ...
    stripInlineColor: stripInlineColor,
    renderSourceLinkProvenance: renderSourceLinkProvenance,
    customSelectRow: customSelectRow,
    reselectByIndex: reselectByIndex,
    createProduct: createProduct,
    mountPublishStage: mountPublishStage,
    publishFormatRows: publishFormatRows,
  ...
  window.VersoInspectorScopes.install(VE);   // the document / page / multi-selection panels
  ...
  CELL_GEO_LABEL = VE.get("CELL_GEO_LABEL");
  STAGE_IDS = VE.get("STAGE_IDS");
